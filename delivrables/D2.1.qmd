...
This deliverable presents the **D2.1 Repository with Proposed Solutions / Services** of the *Work Package 2 (WP2) Development of a networked portfolio* of the TechTraPlastiCE project, focused on the structuration of a networked portfolio of services, capacities, developments, and training activities related to the circular economy of plastics across partner Higher Education Institutions (HEIs). 
The report establishes a baseline mapping of institutional competencies to support technology transfer, strengthen university–industry collaboration, and foster innovation ecosystems in Latin America and Europe.

The methodology combines structured questionnaires, iterative validation processes, and participatory canvas-based workshops. This mixed-method approach enabled the collection of both standardized and context-sensitive data from a diverse consortium with varying levels of institutional maturity and regional contexts. The resulting dataset captures technical services, research and innovation capabilities, transferable developments, and educational activities connected to circular economy challenges.

The repository generated through WP2 provides a consolidated and comparable overview of partner expertise and resources, creating the foundation for future collaboration and co-creation activities in subsequent work packages. The report also highlights differences in terminology, data structure, and detail among participating institutions, emphasizing the need for further harmonization and analytical refinement. Overall, D2.1 delivers a strategic operational framework for strengthening technology transfer and sustainable innovation in the plastics circular economy sector.
...
#   set_names(c("HEI", "Title-ES","Title-EN","Subtitle-ES","Subtitle-EN","Description-ES","Description-EN","Impacts-ES","Impacts-EN","TRL","Results","Field","Photo","Keywords","Contact"))   %>% 
#   arrange(desc(HEI))  %>% 
#   rowid_to_column("ID")

# datos <- datos %>% 
#   pivot_longer(2:ncol(datos))   %>%  
#   mutate(
#     value = value %>%
#       str_replace_all("&", "\\\\&") %>%
#       str_replace_all("%", "\\\\%") %>%
...
# write.csv(datos, file="delivrables/annex/2.1/Developments.csv")
# rm(datos)


datos <- read_csv(file = here("delivrables/annex/2.1/Developments.csv"))

table1<- datos %>%   
  #mutate(Description-EN = tolower(`Description-EN`))  %>% 
  #rowid_to_column("ID") %>% 
  select(HEI, ID, `Title-EN`, `Subtitle-EN`)  %>% 
  #arrange(HEI) %>%
  mutate(`Subtitle-EN` = str_to_sentence(`Subtitle-EN`))

table1  %>% 
mutate(HEI = factor(HEI))  %>% 
  arrange(HEI) %>%
  mutate_all(linebreak) %>%
...
#   set_names(c("HEI", "Title-ES","Title-EN","Subtitle-ES","Subtitle-EN","Description-ES","Description-EN","Aplications-ES","Aplications-EN","Photo","Keywords","Contact")) %>% 
#   arrange(desc(HEI))  %>% 
#   mutate(HEI = str_to_title(HEI))  %>% 
#   rowid_to_column("ID")
...
# write.csv(datos, file="delivrables/annex/2.1/Capabilities2.csv")
# View(datos)


datos <- read_csv(file= here("delivrables/annex/2.1/Capabilities2.csv"))
...
datos  %>% 
  ...
  kbl("latex", booktabs = T, escape = F, row.names = F,
   ...
   kable_styling(full_width = F, latex_options = c("repeat_header", "scale_down"), font_size = 7) %>% 
   row_spec(0, color = "white", background = "Verde", bold = T, font_size = 7) %>%
   column_spec(1, width = c("3cm"))  %>%
   column_spec(2, width = c("0.6cm"))  %>%
   column_spec(3, width = c("5.5cm")) %>%
   column_spec(4, width = c("9.5cm")) %>%   
...
#   set_names(c("HEI", "Title-ES","Title-EN","Subtitle-ES","Subtitle-EN","Description-ES","Description-EN","Technical-ES","Technical-EN","Applications-ES", "Applications-EN", 
#   "Photo", "Keywords","Contact")) %>% 
#   arrange(desc(HEI))  %>% 
#   mutate(HEI = str_to_title(HEI))  %>% 
#   rowid_to_column("ID")  %>% 
#   pivot_longer(2:15) %>%  
...
# write.csv(datos, file="delivrables/annex/2.1/Technical2.csv")
# names(datos)

datos <- read_csv(file= here("delivrables/annex/2.1/Technical2.csv"))  
#levels(as.factor(datos$HEI))
...
datos %>% 
  ...
  kbl("latex", booktabs = T, escape = F, row.names = F,
   ...
   kable_styling(full_width = F, latex_options = c("repeat_header", "scale_down"), font_size = 7) %>% 
   row_spec(0, color = "white", background = "Celeste", bold = T, font_size = 7) %>%
...
#   set_names(c("HEI", "Title-ES","Title-EN","Content-ES","Content-EN","Description-ES","Description-EN","Area","Modality","Target", "Level", "Workload", "Language",
#   "Photo", "Evaluation", "Certification", "Keywords","Contact")) %>% 
#   arrange(desc(HEI))  %>% 
#   mutate(HEI = str_to_title(HEI))  %>% 
#   rowid_to_column("ID")  %>% 
#   pivot_longer(2:19) %>%  
...
# write.csv(datos, file="delivrables/annex/2.1/Training2.csv")
# names(datos)


datos <- read_csv(file= here("delivrables/annex/2.1/Training2.csv")) 
#View(datos)
...
datos %>% 
  select(HEI, ID, `Title-EN`, `Content-EN`, Level) %>% 
  arrange(desc(HEI)) %>%
  #mutate_all(linebreak) %>%
  kbl("latex", booktabs = T, escape = F, row.names = F,
      longtable = T, 
   ...
   kable_styling(full_width = F, latex_options = c("repeat_header", "scale_down"), font_size = 7) %>% 
   row_spec(0, color = "white", background = "Neutro", bold = T, font_size = 7) %>%
   column_spec(1, width = c("2cm"))  %>%
   column_spec(2, width = c("1cm"))  %>%
   column_spec(3, width = c("3cm")) %>%
   column_spec(4, width = c("11cm")) %>%   
   collapse_rows(columns = 1:1, valign = "top")
   #pack_rows("Group 1", 3, 5, label_row_css = "background-color: #666; color: #fff;")
...
This includes visual records of the completed canvas by each HEIs, which capture collective reflections (@fig-canva2 and @fig-canva3) on the institutional insights, and identified opportunities for collaboration. 
These materials provide additional context and depth to the structured dataset that are  included in the @sec-A2.


:::{#fig-workshop layout="[48,-3,48]"}
...
Socialization of the canva for each HEI
:::
...
\newgeometry{a1paper , left=15mm,right=15mm, top=20mm, bottom=20mm}
...
datos <- read_csv(here("delivrables/annex/2.1/Developments.csv"))

datos %>% 
  select(-1, -Photo, -Contact) %>% 
  ...
  kbl("latex", booktabs = T, escape = F, row.names = F,
   ...
   kable_styling(full_width = F, latex_options = c("repeat_header"), font_size = 3) %>% 
   row_spec(0, color = "white", background = "Naranja", bold = T, font_size = 4) %>%
   column_spec(1, width = c("0.5mm")) %>%  
   column_spec(2, width = c("0.5cm"))  %>%
   column_spec(3, width = c("0.5cm")) %>%
   column_spec(4, width = c("0.5cm")) %>% 
   column_spec(5, width = c("1cm")) %>%  
   column_spec(6, width = c("1cm")) %>%     
   column_spec(7, width = c("3.5cm")) %>%
   column_spec(8, width = c("3.5cm")) %>% 
   column_spec(9, width = c("2.5cm")) %>%
   column_spec(10, width = c("2.5cm")) %>% 
   column_spec(11, width = c("1.5cm")) %>% 
   column_spec(12, width = c("1.5cm")) %>%
   column_spec(13, width = c("1cm")) %>%           
   column_spec(14, width = c("1cm"))
...
datos <- read_csv(here("delivrables/annex/2.1/Capabilities2.csv"))
...
datos %>% 
  ...
  kbl("latex", booktabs = T, escape = F, row.names = F,
   ...
   kable_styling(full_width = F, latex_options = c("repeat_header"), font_size = 3) %>% 
   row_spec(0, color = "white", background = "Verde", bold = T, font_size = 4) %>%
   column_spec(1, width = c("0.5mm")) %>%  
   column_spec(2, width = c("0.5cm"))  %>%
   column_spec(3, width = c("0.5cm")) %>%
   column_spec(4, width = c("0.5cm")) %>% 
   column_spec(5, width = c("1cm")) %>%  
   column_spec(6, width = c("1cm")) %>%     
   column_spec(7, width = c("6cm")) %>%
   column_spec(8, width = c("6cm")) %>% 
   column_spec(9, width = c("1.5cm")) %>%
   column_spec(10, width = c("1.5cm")) %>% 
   column_spec(11, width = c("1cm")) 
   #column_spec(12, width = c("1.5cm")) %>%
   #column_spec(13, width = c("1cm")) %>%           
   #column_spec(14, width = c("1cm"))
...
datos %>% 
  ...
  kbl("latex", booktabs = T, escape = F, row.names = F,
   ...
   kable_styling(full_width = F, latex_options = c("repeat_header"), font_size = 3) %>% 
   row_spec(0, color = "white", background = "Celeste", bold = T, font_size = 4) %>%
   column_spec(1, width = c("0.5mm")) %>%  
   column_spec(2, width = c("0.5cm"))  %>%
   column_spec(3, width = c("0.5cm")) %>%
   column_spec(4, width = c("0.5cm")) %>% 
   column_spec(5, width = c("0.5cm")) %>%  
   column_spec(6, width = c("1cm")) %>%     
   column_spec(7, width = c("3.5cm")) %>%
   column_spec(8, width = c("3.5cm")) %>% 
   column_spec(9, width = c("2.5cm")) %>%
   column_spec(10, width = c("2.5cm")) %>% 
   column_spec(11, width = c("2cm")) %>% 
   column_spec(12, width = c("2.5cm")) %>%
   column_spec(13, width = c("1cm")) %>%           
   column_spec(14, width = c("1.5cm"))
...
datos %>% 
  ...
  kbl("latex", booktabs = T, escape = F, row.names = F,
   ...
   kable_styling(full_width = F, latex_options = c("repeat_header", "scale_down"), font_size = 3) %>% 
   row_spec(0, color = "white", background = "Neutro", bold = T, font_size = 4) %>%
   column_spec(1, width = c("0.5mm")) %>%  
   column_spec(2, width = c("0.5cm"))  %>%
   column_spec(3, width = c("0.5cm")) %>%
   column_spec(4, width = c("0.5cm")) %>% 
   column_spec(5, width = c("2cm")) %>%  
   column_spec(6, width = c("2cm")) %>%     
   column_spec(7, width = c("2cm")) %>%
   column_spec(8, width = c("2cm")) %>% 
   column_spec(9, width = c("1cm")) %>%
   column_spec(10, width = c("1cm")) %>% 
   column_spec(11, width = c("1cm")) %>% 
   column_spec(12, width = c("1cm")) %>%
   column_spec(13, width = c("1cm")) %>%           
   column_spec(14, width = c("0.5cm"))  %>% 
   column_spec(15, width = c("2cm")) %>% 
   column_spec(16, width = c("0.5cm")) %>% 
   column_spec(17, width = c("1cm")) %>% 
   column_spec(18, width = c("1cm"))
